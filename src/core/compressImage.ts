...
const supportMimeTypes = Object.keys(mimeTypes).map((type) => mimeTypes[type as MimeKey])
const defaultType = mimeTypes.JPEG

function isSupportedType(type: typeof mimeTypes[MimeKey]) {
...
const compressImage = (file: File, options: CompressOptions) => {
